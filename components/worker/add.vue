...
<div class="box-header with-border">
  <h3 class="box-title">增加操作员</h3>
</div>
<div class="box-body">
	<form method="post" v-on:submit.prevent="add()" >
	  <div class="form-group">
		<label for="exampleInputEmail1">操作员id</label>
		<input type="text"  class="form-control" v-model="worker.workerId"   required >
	  ...
	  <div class="form-group">
	  		...
	  		<input type="text" class="form-control" v-model="worker.address" required>
	  </div>
...
<script>
	...
	export default {
		...
		methods:{
			add(){
				// document.write(this.worker.role);
				axios.post("http://localhost:8200/worker/add",this.worker).then(result=>{
